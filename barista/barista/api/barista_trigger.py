...
def send_report(run_name):
	...
	if sort_att == 'Module':
		col = 1
		module_tup = frappe.db.sql("""SELECT DISTINCT ts.module AS 'Module' FROM `tabTest Suite` ts INNER JOIN `tabTest Result` tr ON ts.name=tr.test_suite AND tr.test_run_name='{run_name}' WHERE ts.module IS NOT NULL AND ts.module != '';""".format(run_name = run_name))
	elif sort_att == 'Workgroup':
		col = 6
		module_tup = frappe.db.sql("""SELECT DISTINCT ts.workgroup AS 'Workgroup' FROM `tabTest Suite` ts INNER JOIN `tabTest Result` tr ON ts.name=tr.test_suite AND tr.test_run_name='{run_name}' WHERE ts.workgroup IS NOT NULL AND ts.workgroup != '';""".format(run_name = run_name))
	elif sort_att == 'SPOC':
		col = 7
		module_tup = frappe.db.sql("""SELECT DISTINCT ts.spoc AS 'SPOC' FROM `tabTest Suite` ts INNER JOIN `tabTest Result` tr ON ts.name=tr.test_suite AND tr.test_run_name='{run_name}' WHERE ts.spoc IS NOT NULL AND ts.spoc != '';""".format(run_name = run_name))
...
# Create Test-Data from document
@frappe.whitelist()
def create_test_data_from_doc(doctype, docname):
	# DocFields to ignore:
	no_value_fieldtypes = (
		"Section Break",
		"Column Break",
	...
	# DocFields to iterate and create separate Test Data for each entry:
	childtable_fieldtypes = (
		"Table",
		"Table Multiselect"
	)

	# Create basic Test Data doc:
	doc = frappe.get_doc(doctype, docname)
	test_data_doc = frappe.get_doc({
		"doctype": "Test Data",
		"doctype_name": doctype,
		"is_standard": "Yes",
		"app_name": frappe.db.get_value("Module Def", doc.meta.module, "app_name"),
		"doctype_type": "Master",
		"create_using": "Data"
	})

	# Check each field of document and add in Test Data doc:
	for row in doc.meta.fields:
		# Handle Child Tables:
		if row.fieldtype in childtable_fieldtypes:
			for table_entry in doc.get(row.fieldname):
				test_data = create_test_data_from_doc(row.options, table_entry.name)
				test_data_doc.append("docfield_value", {
					"doctype_name": doctype,
					"docfield_fieldname": row.fieldname,
					"docfield_code_value": "Fixed Value",
					"linkfield_name": test_data.get('name', ''),
					"linkfield_key": "name"
				})
		elif row.fieldtype not in no_value_fieldtypes:
			# Handle Links:
			if row.fieldtype == "Link" and doc.get(row.fieldname) and row.options!= "DocType":
				link_test_data = create_test_data_from_doc(row.options, doc.get(row.fieldname))
				test_data_doc.append("docfield_value", {
				"doctype_name": doctype,
				"docfield_fieldname": row.fieldname,
				"docfield_code_value": "Fixed Value",
				"linkfield_name": link_test_data.get('name', ''),
				"linkfield_key": "name"
				})
			# Handle Dynamic Links:
			elif row.fieldtype == "Dynamic Link" and doc.get(row.fieldname) and doc.get(row.options) != "DocType":
				dl_test_data = create_test_data_from_doc(doc.get(row.options), doc.get(row.fieldname))
				test_data_doc.append("docfield_value", {
				"doctype_name": doctype,
				"docfield_fieldname": row.fieldname,
				"docfield_code_value": "Fixed Value",
				"linkfield_name": dl_test_data.get('name', ''),
				"linkfield_key": "name"
				})
			# Normal DocField:
			test_data_doc.append("docfield_value", {
				...
				"docfield_fieldname": row.fieldname,
				"docfield_code_value": "Fixed Value",
				"docfield_value": doc.get(row.fieldname),
			})
	test_data_doc.save(True)

	return test_data_doc
